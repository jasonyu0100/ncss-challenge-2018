...
class Dealer:
  ...
  def getOptimalCombinations(hand):
    ...
    optimalCombinations[1] += [[two] for two in twos]
...
class BigTwoBot:
  ...
  def winPercentageStrategy(self,hand, isStartOfRound, playToBeat, roundHistory, playerNo, handSizes, scores, roundNo):
    ...
    if len(playToBeat) == 0:
      if '3D' in hand: return Dealer.find3DiamondCombination(optimalCombinations)
      else: return Dealer.firstRoundPlay(optimalCombinations,unplayed,hand)
    else:
...
wins = Dealer.testing(players,1000,True)
print(wins)
