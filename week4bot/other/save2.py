...
def play(hand, isStartOfRound, playToBeat, roundHistory, playerNo, handSizes, scores, roundNo):
  bot = BigTwoBot(0,BigTwoBot.winningStrategy,BotGenes(BotGenes.THIRD_BEST_GENES))
...
class Dealer:
  ...
  def getPlayableComboTypes(comboType):
    hierarchy = ['SINGLE','DOUBLE','TRIPLE','STRAIGHT','FLUSH','FULL','QUAD','STRAIGHT_FLUSH']
...
class BotGenes:
  ...
  SECOND_BEST_GENES = {'STRAIGHT_FLUSH': 35.1494140625, 'FLUSH': 25.7958984375, 'STRAIGHT': 29.0771484375, 'QUAD': 27.39453125, 'FULL': 29.2626953125, 3: 28.9453125, 2: 28.1884765625, 1: 25.2998046875}
  ...
  def generateGenes(self):
    self.genes = {
      ...
      3:random.randint(0,52),
      2:random.randint(0,52),
      1:random.randint(0,52)
    }
...
class BotPopulation:
  ...
  def calculateFitness(self):
    players = [
    None,
    BigTwoBot(1,BigTwoBot.winningStrategy,),
    BigTwoBot(2,BigTwoBot.winningStrategy,),
    BigTwoBot(3,BigTwoBot.winningStrategy,),
    ]
...
# pop = BotPopulation(25,25)
# best = pop.trainPopulation(50)
# print(best.genes)

players = [
          BigTwoBot('0',BigTwoBot.winningStrategy,BotGenes(BotGenes.THIRD_BEST_GENES)),
